...
def register_scripts(r: aioredis.Redis) -> None:
...
class Multilock(object):
    ...
    def __init__(
        self, redis: aioredis.Redis, name: str, retry_count=None, retry_delay=None
    ):
        logging.debug(
    ...
    async def acquire_exclusive(self, ttl: float, retry_count=None, retry_delay=None):
        logging.debug(
        ...
        while retry <= retry_count:
            retcode = await acquire_exclusive_lua_script(
                args=[self.name, self._id, now, ttl]
            )
    ...
    async def acquire_shared(self, ttl: float, retry_count=None, retry_delay=None):
        logging.debug(
        ...
        retry = 0
        while retry <= retry_count:
            retcode = await acquire_shared_lua_script(
                args=[self.name, self._id, now, ttl]
            )
            logging.debug("    - retcode: %r", retcode)
    ...
    async def refresh(self, ttl: float) -> None:
        ...
        if self._state == LockState.ACQUIRED_EXCLUSIVE:
            retcode = await refresh_exclusive_lua_script(
                args=[self.name, self._id, now, ttl]
            )
            logging.debug("    - retcode: %r", retcode)
            ...
            retcode = await refresh_shared_lua_script(
                args=[self.name, self._id, now, ttl]
            )
            logging.debug("    - retcode: %r", retcode)
    ...
    async def release(self) -> None:
        ...
        if self._state == LockState.IDLE:
            ...
            retcode = await release_exclusive_lua_script(args=[self.name, self._id])
            logging.debug("    - retcode: %r", retcode)
            ...
            retcode = await release_shared_lua_script(args=[self.name, self._id])
            logging.debug("    - retcode: %r", retcode)
    ...
    async def flush(self) -> None:
        ...
        retcode = await flush_lua_script(args=[self.name])
        logging.debug("    - retcode: %r", retcode)
